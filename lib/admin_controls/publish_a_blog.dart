...
// Create a unique file name for image
  ...
  postBlogToFirestore() async
  {
    ...
    BlogModel blogModel = BlogModel(imageUrl: imageUrl, description: descriptionEditingController.text, date: date, time: time);

    // BlogModel blogModel = BlogModel();
    //
    // blogModel.imageUrl = imageUrl;
    // blogModel.description = descriptionEditingController.text;
    // blogModel.date = date;
    // blogModel.time = time;

    await firebaseFirestore.collection("blogs").doc(blogid).set(blogModel.toMap());



    //await firebaseFirestore.collection("users").doc(uid).set(blogModel.toMap());

    Fluttertoast.showToast(msg: "Blog Published successfully.");

    //Navigator.pushAndRemoveUntil(context as BuildContext, MaterialPageRoute(builder: (context) => LoginScreen()), (route) => false);
    Navigator.pushAndRemoveUntil(context as BuildContext, MaterialPageRoute(builder: (context) => adminScreen()), (route) => false);
